...
function gpsindy_dbl_lasso_int( x_train_GP, dx_train_GP, t_train_x2, u_train_x2, λ, data_train, data_test )  
    ...
    t_train, x_gpsindy_train = downsample( data_train.t, t_train_x2, x_gpsindy_train ) 
...
function dx_Ξ_integrate( data_train, data_test, Ξ, x0_train, x0_test ) 
    ...
    x_train_sindy = integrate_euler( dx_fn_sindy, x0_train, data_train.t, data_train.u ) 
    x_test_sindy  = integrate_euler( dx_fn_sindy, x0_test, data_test.t, data_test.u ) 
...
function ode_train_test( fn, noise = 0.01 ) 
    ...
    # standardize data 
    data_train_stand = ode_stand_data( data_train, fn, p, noise ) 

    return data_train, data_test, data_train_stand 
end 
